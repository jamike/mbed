...
* Return Value : none
*******************************************************************************/
uint16_t usb0_function_set_pid_nak (uint16_t pipe)
{
    uint16_t pid;
    uint16_t pbusy;
    uint32_t loop;
    ...
    greentea_send_kv("TEST_Start5", 0);
    pid = usb0_function_get_pid(pipe);
    ret = pid;
    ...
    greentea_send_kv("TEST_Start6", 0);
    switch (pipe)
    {
        case USB_FUNCTION_PIPE0:
            RZA_IO_RegWrite_16(&USB200.DCPCTR,
...
* Return Value : none
*******************************************************************************/
uint16_t usb0_function_clear_pid_stall (uint16_t pipe)
{
    uint16_t ret;
    
    greentea_send_kv("TEST_Start4", 0);
    ret = usb0_function_set_pid_nak(pipe);
